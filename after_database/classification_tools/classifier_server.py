...
def load_snapshot_data(snapshot_image_folder_path, snapshot_metadata_file_path):
    ...
    snapshot_frame_index = snapshot_metadata.get("frame_index")
...
def get_bounding_snap_counts(object_metadata):
    ...
    start_snapshot_count = max(1, object_metadata.get("snapshots").get("first").get("count") - 1)
    end_snapshot_count = object_metadata.get("snapshots").get("last").get("count")    
...
def overview_route():
    ...
    for each_task in task_name_list:
        supervised_labels_list = list(supervised_class_labels_dict.get(each_task).values())
...
def object_label_request(task_name, object_id):
    ...
    object_class_label = supervised_class_labels_dict.get(task_name).get(object_id)
    object_class_index = label_to_idx_lut.get(object_class_label)
...
def object_label_update():
    ...
    new_class_label = update_json_data.get("new_label_string")
    ...
    supervised_class_labels_dict.get(task_name).update(new_entry)
