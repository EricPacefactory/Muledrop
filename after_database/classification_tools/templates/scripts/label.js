...
//
// label_lut_dict:
//      dictionary -> Keys represent class indicies and corresponding values hold class strings
//
// *** DATA ROUTES
...
function update_label_state_indication(label_string){
    ...
    // or otherwise reset to default class
    const labels_ul = document.getElementById("label_class_buttons_ul");
    const directives_ul = document.getElementById("label_directive_buttons_ul");
    const uls_to_update = [labels_ul, directives_ul];
    for (each_ul of uls_to_update) {
        const li_nodes = each_ul.children;

        // Highlight the class label button whose text matches the currently selected task name
        // or otherwise reset to default class
        for (each_li of li_nodes) {
            const li_text = each_li.innerText;
            const new_class = (li_text == label_string) ? active_class : default_class;
            each_li.className = new_class;
        }
    }
...
function label_request(task_name, object_id, debug_feedback = true){
    ...
    */
    ...
    .then(response_data => {
        ...
        const label_string = response_data["label_string"];
        update_label_state_indication(label_string);
    });
...
function post_label_update_builder(label_string){
    ...
    */
    ...
    function post_label_update(){
        ...
        const new_label_string = label_string;
        ...
        update_json = {"task_name": task_name, "object_id": object_id, "new_label_string": new_label_string}
        const fetch_param = {
        ...
        update_label_state_indication(new_label_string);
...
function initialize(){
    ...
    for(let k = 0; k < label_buttons.length; k++){
        let btn_ref = label_buttons.item(k);
        let label_str = btn_ref.innerText;
        console.log(label_buttons.item(k).innerText);
        btn_ref.addEventListener("click", post_label_update_builder(label_str));
    }
