...
for each_snap_time in all_snap_times:
    ...
    snap_wh = (snap_md.get("snap_width"), snap_md.get("snap_height"))
    snap_datetime_isoformat = snap_md.get("datetime_isoformat")
    ...
    for each_task in task_list:
        ...
        for each_obj_id in obj_id_list:
            ...
            class_label = supervised_labels_dict.get(each_task).get(each_obj_id)
            
            # Retrieve object reconstruction data, if available, otherwise generate (and store) it now
            obj_ref = obj_recon_dict.get(each_task).get(each_obj_id)
...
for each_curation_path in curation_file_paths:
    ...
    snap_name = loaded_curation_dict.get("snapshot_metadata").get("name")
    snap_time = loaded_curation_dict.get("snapshot_metadata").get("epoch_ms_utc")
    ...
    task_data = loaded_curation_dict.get("tasks")
    for each_task_name, each_task_dict in task_data.items():
        
        for each_obj_id, each_obj_md in each_task_dict.items():
        
            # Draw object bounding box
            pt1, pt2 = np.int32(np.round(each_obj_md.get("box_tlbr") * snap_frame_scaling))
            ...
            label_text = each_obj_md.get("class_label")
