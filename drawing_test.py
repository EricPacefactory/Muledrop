...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 26 17:07:11 2019
...
class Entity_Drawer:
    ...
    def __init__(self, 
        ...
        self.border_size_px = int(border_size_px)
        self.min_entities = minimum_entities
        self.max_entities = maximum_entities
        self.min_points_per_entity = max(2, minimum_points)
        self.max_points_per_entity = maximum_points
    ...
    def _normalize(self, pixelized_points_array):
        ...
        frame_scaling = np.float32((frame_h - 1, frame_w - 1))
    ...
    def entity_list(self):
        ''' Return a (normalized) list-of-lists-of-tuples representing point xy co-ordinates '''        
    ...
    def mouse_callback(self, event, mx, my, flags, param):
        ...
        mouse_state_func = self.mouse_state_callbacks.get(self.state)
    ...
    def keypress_callback(self, keypress_ex):
        ...
        It must be called in a loop using the keypress output from OpenCV's keypressEx result:
            ...
            keypress_ex = cv2.waitKeyEx(1)            
            this_object_instance.keypress_callback(keypress_ex)
        ...
        if keypress_ex != -1:
            ...
            # Split keypress into a usable key code + potential modifier (e.g. shift, alt, ctrl etc.) code
            ...
            key_state_func = self.keypress_state_callbacks.get(self.state)
            key_state_func(keycode, mod_shift, mod_ctrl, mod_alt, self.last_mouse_xy)
...
class Interactive_Entity:
    ...
    def __repr__(self):
        ...
        repr_strs = ["Entity",
                     ...
                     "          Min xy: ({}, {})".format(*np.min(self.points(), axis=0)),
...
def keypress_ex_quit(keypress_ex, quit_on_esc = True, quit_on_q = True):
    ...
    keycode, _ = keypress_ex_decoder(keypress_ex)
...
def keypress_modifier_decoder(modifier_code):
    ...
    mod_shift = modifier_code &     0b00000001
    mod_capslock = modifier_code &  0b00000010
    mod_ctrl = modifier_code &      0b00000100
    mod_alt = modifier_code &       0b00001000
    mod_numlock = modifier_code &   0b00010000
    # missing modifier key
    mod_super = modifier_code &     0b01000000
    # missing modifier key
    
    return mod_shift, mod_capslock, mod_ctrl, mod_alt, mod_numlock, mod_super
...
draw_spec = {"drawing": {"entity_type": "polygon",      # Set by configurable, goes to drawer
                         "drawing_style": "zone",       # Goes to wrapper drawer
                         "out_of_bounds": True,         # Goes to drawer?
                         "min_max_entities": (0, 1),    # Goes to drawer
                         "min_max_points": (4, 4),      # Goes to drawer
                         "variable_name": "mask_zones"  # Tells wrapper how to set variables
                         }
             }
...
# FOR TESTING
'''
test_points = [[(250, 100), (350, 100), (400, 250), (270, 200)],
                [(448, 433), (176, 248), (236, 419), (90, 295), (98, 127)]]
'''
...
'''
def new_draw(frame, points):
    cv2.polylines(frame, [points], False, (255, 255, 255), 2, cv2.LINE_4)
    for each_point in points:
        cv2.circle(frame, tuple(each_point), 15, (255, 0, 170), 2, cv2.LINE_8)
        
def weird_draw(frame, points):
    
    for each_point in points:
        cv2.rectangle(frame, (0,0), tuple(each_point), (120, 0, 77), 2, cv2.LINE_4)
        cv2.circle(frame, tuple(each_point), 15, (255, 0, 170), 2, cv2.LINE_8)

drawer.replace_drawing_functions(weird_draw, new_draw)
'''
...
'''
STOPPED HERE
- NEED TO CONSIDER HOW TO GET DATA OUT OF DRAWER NICELY!
    - Done? (.entity_list property)
- then need to think about how drawing could be specified from json input
- then think about how to 'attach' drawing to a window, based on json spec
- finally, need to figure out mouse follower setup (should it be a different class?)
    - would make sense, since follower isn't similar to entity idea...
'''
...
while True:
    ...
    # Get keypresses
    keypress_ex = cv2.waitKeyEx(10)
    if keypress_ex_quit(keypress_ex):
        break
    
    drawer.keypress_callback(keypress_ex)
