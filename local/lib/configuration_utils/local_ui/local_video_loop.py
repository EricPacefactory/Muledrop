...
def local_rule_config_video_loop(video_reader_ref,
                                 background_capture_ref,
    ...
    # Generate stage timing window
    #timing_window = Local_Rule_Timing_Window(core_proccess_sequence)
    ...
    while True:
        ...
        if req_break:
            # Loop the video back at the end if we miss a frame
            video_reader_ref.set_current_frame(0)
            core_proccess_sequence.reset_all()
            continue
        ...
        values_changed_dict = local_controls.read_all_controls()
        if values_changed_dict:
            rule_ref.setup(values_changed_dict)
        ...
        skip_frame, stage_outputs, stage_timing = \
        core_proccess_sequence.run_debuggable(bg_outputs, current_time_elapsed, current_datetime)
