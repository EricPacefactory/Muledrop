...
class Configurable(Reference_Background_Capture):
    ...
    def generate_background_from_resources(self,
        ...
        # Finally, try to create a background image by taking the median of corresponding pixels along all captures
        try:
            new_background_image = np.uint8(np.round(np.median(frame_stack, axis = 0)))
            ...
            print("Error generating median background! ({})".format(error_type), err, sep = "\n")
            new_background_image = None
