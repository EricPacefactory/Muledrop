...
class Frame_Processor_Stage(Reference_Frame_Processor):
    ...
    def __init__(self, input_wh):
        ...
        self._next_rbg_update_time_sec = -1.0
        ...
        bg = self.controls_manager.new_control_group("Background Controls")
        
        self.capture_period_sec = \
        bg.attach_slider("capture_period_sec", 
        ...
        bg.attach_slider("new_capture_weighting", 
    ...
    def reset(self):
        ...
        self._rolling_bg_frame_float32 = None
        self._next_rbg_update_time_sec = -1
    ...
    def setup(self, variables_changed_dict):
        ...
        self._rbg_func_list, self._proc_func_list, (self.output_w, self.output_h) = self._build_frame_processor()
        
        # Update the background image if possible
        self._apply_rbg_frame_processing()
    ...
    def _update_rolling_background(self, frame):
        ...
        __, current_time_sec, _ = self.get_time_info()
        
        # Bail if we don't need to update the background
        need_to_update_rolling_bg = (current_time_sec > self._next_rbg_update_time_sec)
        if not need_to_update_rolling_bg:
            return
            
        # If we get here we're updating the background so setup the next update time
        self._next_rbg_update_time_sec = current_time_sec + self.capture_period_sec
        ...
        # Convert to uint8 for use in frame processing
        new_rbg_uint8 = np.uint8(np.round(self._rolling_bg_frame_float32))
        self._clean_rolling_bg_uint8 = new_rbg_uint8.copy()
        
        # Update the frame data used for frame processing
        self._apply_rbg_frame_processing()
            
    # .................................................................................................................
    
    def _apply_rbg_frame_processing(self):
        
        ''' 
        Function for converting the clean rolling background to the actual frame data 
        used in background subtractio, which may include scaling/blurring
        '''
