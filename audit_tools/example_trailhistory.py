...
class Object_Reconstruction:
    ...
    def __init__(self, object_metadata, frame_wh, time_bar_wh, global_start_index, global_end_index, 
        ...
        self.num_samples = self.metadata.get("tracking").get("num_samples")
        ...
        self.start_idx = self.metadata.get("timing").get("start_frame_index")
    ...
    def get_bounding_snap_counts(self, padding = 3):
        ...
        start_count = snap_metadata.get("start").get("snapshot_count")
        end_count = snap_metadata.get("last").get("snapshot_count")
...
def load_snapshot_image_from_metadata(snapshot_image_folder, path_to_metadata_file):
    ...
    snapshot_frame_index = snap_metadata.get("snapshot_frame_index")
    snap_name = snap_metadata.get("snapshot_name")
...
def get_snapshot_startend_timing(snap_data_paths_list):
    ...
    first_frame_index = first_snap_data.get("snapshot_frame_index")
    
    last_snap_path = snap_data_paths_list[-1]
    last_snap_data = load_metadata(last_snap_path)
    last_frame_index = last_snap_data.get("snapshot_frame_index")
