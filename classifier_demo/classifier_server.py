...
from local.lib.file_access_utils.reporting import build_image_report_path
...
from eolib.utils.read_write import save_json, load_json, update_json
...
# ADD TO FILE_ACCESS_UTILS!!!
def build_classifier_path(cameras_folder_path, camera_select, *path_joins):
...
def build_dataset_path(cameras_folder_path, camera_select, *path_joins):
    classifier_folder_path = build_classifier_path(cameras_folder_path, camera_select)
...
# ADD TO FILE_ACCESS_UTILS!!!
def build_labels_path(cameras_folder_path, camera_select, dataset_select, *path_joins):    
    return build_dataset_path(cameras_folder_path, camera_select, dataset_select, "labels", *path_joins)

# .....................................................................................................................

# ADD TO FILE_ACCESS_UTILS!!!
def build_label_listing_path(cameras_folder_path, camera_select, dataset_select): 
    return build_dataset_path(cameras_folder_path, camera_select, dataset_select, "label_list.json")
...
def prompt_to_add_dataset(cameras_folder, camera_select):
    ...
    source_copy_folder = users_with_data.get(user_select)
...
def load_snapshot_data(snapshot_image_folder_path, snapshot_metadata_file_path):
    ...
    snapshot_frame_index = snapshot_metadata.get("snapshot_frame_index")
...
def get_snapshot_luts(snapshot_metadata_folder_path, snapshot_metadata_file_list):
    ...
    for each_snap_file in snapshot_metadata_file_list:
        ...
        snap_count = snap_md.get("snapshot_count")
        snap_frame_index = snap_md.get("snapshot_frame_index")
...
def annotate_trail(frame, object_metadata, final_plot_index):
    ...
    num_samples = object_metadata.get("tracking").get("num_samples")
...
def annotate_hull(frame, object_metadata, final_plot_index):
    ...
    hull = object_metadata.get("tracking").get("hull")[final_plot_index]
...
def get_bounding_snap_counts(object_metadata):
    ...
    start_snapshot_count = max(1, object_metadata.get("snapshots").get("start").get("snapshot_count") - 1)
    end_snapshot_count = object_metadata.get("snapshots").get("last").get("snapshot_count")    
...
def get_example_annotated_image(task_select, object_id, relative_snapshot_index = 0.5):
    ...
    obj_md_path = task_objids_path_luts.get(task_select).get(object_id)    
    ...
    snap_path = snap_count_luts.get(target_snap_count).get("path")
    ...
    obj_end_frame_index = obj_md.get("timing").get("last_frame_index")
...
def animation_generator(task_name, object_id):
    ...
    # Get pathing to object metadata using the lut, then load the json data
    obj_md_path = task_objids_path_luts.get(task_name).get(object_id)
    obj_md = load_json(obj_md_path)
    ...
    snap_path_list = [snap_count_luts.get(each_count).get("path") for each_count in snap_count_range]
...
label_records_folder_path = build_labels_path(cameras_folder_path, camera_select, dataset_select)
os.makedirs(label_records_folder_path, exist_ok = True)

# Create the label lookup table, if it doesn't already exist
label_listing_file_path = build_label_listing_path(cameras_folder_path, camera_select, dataset_select)
if not os.path.exists(label_listing_file_path):
    default_lut = {0: "unclassified", 1: "ignore", 2: "unknown", 3: "pedestrian", 4: "vehicle"}
    ordered_label_list = [default_lut.get(each_key) for each_key in sorted(default_lut)]
    save_json(label_listing_file_path, ordered_label_list, sort_keys=True)
ordered_label_list = load_json(label_listing_file_path)
...
for each_task, each_obj_id_path_lut in task_objids_path_luts.items():
    empty_class_label_dict = {each_id: 0 for each_id in each_obj_id_path_lut.keys()}
    class_labels_dict.update({each_task: empty_class_label_dict})

# Update/overwrite 'empty' classification labels with any existing data
for each_task, each_label_dict in class_labels_dict.items():
    
    # Load in any existing data to overwrite the default 'empty' labels
    new_label_file_path = os.path.join(label_records_folder_path, each_task)
...
def overview_route():
    ...
    example_snap_path = snap_count_luts.get(example_snap_count).get("path")
    example_snapshot_metadata = load_json(example_snap_path)
    video_source = example_snapshot_metadata.get("video_select", "unknown video!")
...
def label_route():
    ...
    index_label_tuples = [(each_idx, each_label) for each_idx, each_label in enumerate(ordered_label_list)]
    
    return render_template("label.html", task_id_lists = task_id_lists, ordered_label_list = index_label_tuples)
...
def object_label_request(task_name, object_id):
    ...
    object_label_index = class_labels_dict.get(task_name).get(object_id)
    object_label_string = ordered_label_list[object_label_index]    
    label_request_data = {"label_index": object_label_index,
                          "label_string": object_label_string}
...
def object_label_update():
    ...
    # Pull out post data
    task_name = update_json_data.get("task_name")
    object_id = update_json_data.get("object_id")
    new_label_index = update_json_data.get("new_label_index")
    
    # Build new json entry for the labels file
    new_entry = {object_id: new_label_index}
    
    # Get path to the corresponding task label listing and update it with the new object label
    label_file_path = os.path.join(label_records_folder_path, task_name)
    ...
    class_labels_dict.get(task_name).update(new_entry)
    
    # Some debugging feedback
    print("", 
          "LABELUPDATE POST - {} [obj_id: {}, label_idx: {}]".format(task_name, object_id, new_label_index), 
          "", sep="\n")
